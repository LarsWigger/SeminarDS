...
# Dataset
img_ds = Datasets.RandomPhotoAndMonetDataset(normalize=False)
img_dl = DataLoader(img_ds, batch_size=1, pin_memory=True)
sample_photo = torch.reshape(Datasets.PhotoDataset(normalize=False)[4], [1,3,256,256]) # shape must match the input expected by the network

# Training parameters

name = "base_no_normalization" # name of the attempt
